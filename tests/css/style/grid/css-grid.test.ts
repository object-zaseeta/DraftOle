...
describe('CSSGrid', () => {
  ...
  // ── setGrid ショートハンド (Req 4.1, 4.3, 4.4, 4.5, 7.1) ──

  describe('setGrid ショートハンド', () => {
    it('options 未指定の場合、display: grid のみを出力する', () => {
      const sut = makeSUT();
      sut.setGrid();
      const output = sut.render();
      expect(output).toContain('display: grid;');
      expect(output).toBe('display: grid;');
    });

    it('columns: number を repeat(N, 1fr) に変換する', () => {
      const sut = makeSUT();
      sut.setGrid({ columns: 3 });
      const output = sut.render();
      expect(output).toContain('display: grid;');
      expect(output).toContain('grid-template-columns: repeat(3, 1fr);');
    });

    it('columns: string はそのまま透過される', () => {
      const sut = makeSUT();
      sut.setGrid({ columns: '1fr 2fr' });
      const output = sut.render();
      expect(output).toContain('grid-template-columns: 1fr 2fr;');
    });

    it('全オプション（columns/rows/gap/areas/autoFlow）が対応 CSS プロパティに反映される', () => {
      const sut = makeSUT();
      sut.setGrid({
        columns: '1fr',
        rows: '100px',
      ...
      expect(output).toContain('grid-template-rows: 100px;');
      expect(output).toContain('grid-gap: 16px;');
      expect(output).toContain('grid-template-areas: "a b";');
      expect(output).toContain('grid-auto-flow: row;');
    });

    it('既存 setter 互換: setGridTemplateColumns 単独呼び出しでは display は自動付与されない', () => {
      const sut = makeSUT();
      sut.setGridTemplateColumns('1fr 2fr');
      const output = sut.render();
      expect(output).toBe('grid-template-columns: 1fr 2fr;');

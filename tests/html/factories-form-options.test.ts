/**
 * Task 4.3: factories-form の factory 群が末尾の `HtmlTagOptions` を
 * 受け取り、生成されるタグに `css` / `jqm` を注入できることを検証する。
 *
 * Requirements: 4.2, 4.4, 6.1, 6.3
 */
import { describe, it, expect, vi } from 'vitest';
import { form, button, label } from '../../src/html/tags/factories-form.js';
...
describe('factories-form: options 末尾引数のサポート', () => {
  it('form({ css, jqm }) で css/jqm を注入できる', () => {
    const mockCss = createMockCss();
    const mockJqm = createMockJqm();
    const tag = form({ css: mockCss, jqm: mockJqm });
    expect(tag.css).toBe(mockCss);
    expect(tag.jqm).toBe(mockJqm);
  });

  it('button({ id }, "Click", { css }) で attribute/child を保持しつつ options を注入できる', () => {
    const mockCss = createMockCss();
    const tag = button({ id: 'submit' }, 'Click', { css: mockCss });
    expect(tag.css).toBe(mockCss);
    const rendered = tag.render();
    expect(rendered).toContain('id="submit"');
    expect(rendered).toContain('Click');
  });

  it('options を省略した場合は従来通り動作する（後方互換）', () => {
    const tag = label({ class: 'lbl' }, 'Name');
    expect(tag.css).toBeDefined();
    expect(tag.jqm).toBeDefined();
    const rendered = tag.render();
    expect(rendered).toContain('class="lbl"');

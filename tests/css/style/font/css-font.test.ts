...
import { CSSFont, type FontOptions } from '../../../../src/css/style/font/css-font.js';
...
describe('CSSFont', () => {
  ...
  describe('setFont（集約 setter）', () => {
    ...
    it('setFont({size, weight}) が font-size と font-weight をアルファベット順で出力する', () => {
      const sut = makeSUT();
      sut.setFont({ size: '14px', weight: 'bold' });
      expect(sut.render()).toBe('font-size: 14px;\nfont-weight: bold;');
    });

    it('setFont({size}) は font-family/font-weight/line-height/color を出力しない', () => {
      const sut = makeSUT();
      sut.setFont({ size: '14px' });
      const rendered = sut.render();
      expect(rendered).toBe('font-size: 14px;');
      expect(rendered).not.toContain('font-family');
      expect(rendered).not.toContain('font-weight');
      expect(rendered).not.toContain('line-height');
      expect(rendered).not.toContain('color');
    ...
    it('setFont(undefined) は何も出力しない', () => {
      const sut = makeSUT();
      sut.setFont(undefined);
      expect(sut.render()).toBe('');
    });
    ...
    it('setFont が this を返す（identity check）', () => {
      const sut = makeSUT();
      const result = sut.setFont({ size: '14px' });
      expect(result).toBe(sut);
    });

    it('setFont が引数なしでも this を返す', () => {
      const sut = makeSUT();
      const result = sut.setFont();
      expect(result).toBe(sut);
    });

    it('setFont は全フィールド（size, weight, family, lineHeight, color）を一度に設定できる', () => {
      const sut = makeSUT();
    ...
    it('DEV モード: setFontSize 後に setFont({size}) で DuplicateCssPropertyError(font-size)', () => {
      process.env.DRAFT_OLE_DEV = 'true';
      const sut = makeSUT();
      sut.setFontSize('14px');
      expect(() => sut.setFont({ size: '16px' })).toThrow(DuplicateCssPropertyError);
      expect(() => {
        const sut2 = makeSUT();
        sut2.setFontSize('14px');
        sut2.setFont({ size: '16px' });
      }).toThrow(/font-size/);
    });

    it('DEV モード: setFont({size}) 連続呼び出しで DuplicateCssPropertyError(font-size)', () => {
      process.env.DRAFT_OLE_DEV = 'true';
      const sut = makeSUT();
      sut.setFont({ size: '14px' });
      expect(() => sut.setFont({ size: '16px' })).toThrow(DuplicateCssPropertyError);
      expect(() => {
        const sut2 = makeSUT();
        sut2.setFont({ size: '14px' });
        sut2.setFont({ size: '16px' });
      }).toThrow(/font-size/);
    });

    it('DEV モード: 単一 setFont({size, weight}) は throw しない', () => {
      process.env.DRAFT_OLE_DEV = 'true';
      const sut = makeSUT();
      expect(() => sut.setFont({ size: '14px', weight: 'bold' })).not.toThrow();
      expect(sut.render()).toBe('font-size: 14px;\nfont-weight: bold;');
    });
  });
});

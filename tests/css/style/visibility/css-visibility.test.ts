...
describe('CSSVisibility', () => {
  ...
  describe('overflow-x / overflow-y', () => {
    it('overflow-x を設定できる', () => {
      const sut = makeSUT();
      sut.setOverflowX('scroll');
      expect(sut.render()).toBe('overflow-x: scroll;');
    });

    it('overflow-y を設定できる', () => {
      const sut = makeSUT();
      sut.setOverflowY('hidden');
      expect(sut.render()).toBe('overflow-y: hidden;');
    });

    it('overflow-x と overflow-y を個別に設定できる', () => {
      const sut = makeSUT();
      sut.setOverflowX('scroll');
      sut.setOverflowY('hidden');
      const rendered = sut.render();
      expect(rendered).toContain('overflow-x: scroll');
      expect(rendered).toContain('overflow-y: hidden');
    });
  });
  ...
  describe('メソッドチェーン', () => {
    ...
    it('setOverflowX が this を返す', () => {
      const sut = makeSUT();
      const result = sut.setOverflowX('scroll');
      expect(result).toBe(sut);
    });

    it('setOverflowY が this を返す', () => {
      const sut = makeSUT();
      const result = sut.setOverflowY('hidden');
      expect(result).toBe(sut);
    });
    ...
    it('複数のsetterをチェーンして設定できる', () => {
      ...
      expect(rendered).toContain('overflow: hidden');
    });
  });
  ...
  describe('プロパティソート順（アルファベット順）', () => {
    it('複数プロパティがアルファベット順にソートされる', () => {
      ...
      // アルファベット順: clear < display < height < overflow < visibility < width < z-index
      expect(lines[0]).toBe('clear: both');
      expect(lines[1]).toBe('display: flex');
      expect(lines[2]).toBe('height: 200px');
      expect(lines[3]).toBe('overflow: hidden');
      expect(lines[4]).toBe('visibility: visible');
      expect(lines[5]).toBe('width: 100px');
      expect(lines[6]).toBe('z-index: 10;'); // 最後のエントリにはセミコロンが付く
    });

    it('overflow 関連がアルファベット順に並ぶ', () => {
      const sut = makeSUT();
      sut.setOverflowY('hidden');
      sut.setOverflow('auto');
      sut.setOverflowX('scroll');
      ...
      expect(lines[0]).toBe('overflow: auto');
      expect(lines[1]).toBe('overflow-x: scroll');
      expect(lines[2]).toBe('overflow-y: hidden;');
    });
  ...
  describe('設定プロパティのみ出力', () => {
    ...
    it('overflow 関連を部分的に設定した場合、設定分のみ出力される', () => {
      const sut = makeSUT();
      sut.setOverflowX('scroll');
      const rendered = sut.render();
      expect(rendered).toBe('overflow-x: scroll;');
      expect(rendered).not.toContain('overflow-y');
      expect(rendered).not.toContain('overflow:');
    });
  });
  ...
  describe('エッジケース', () => {
    ...
    it('全プロパティを設定した場合のrender()が正常に動作する', () => {
      ...
      expect(rendered).toContain('overflow: hidden');
      expect(rendered).toContain('overflow-x: scroll');
      expect(rendered).toContain('overflow-y: auto');

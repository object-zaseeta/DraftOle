...
> **2026-04-20訂正 (1)**: 当初「Web版SwiftUI」と表現していたが、SwiftUIの本質はリアクティブな状態管理（`@State` / `@Binding` / `@Observable`）であり、宣言的構文はその表面でしかない。DraftOleはリアクティビティをHTMXに委譲する設計のため、正確には **「SwiftUIライクなDXを持つサーバーサイドHTML DSL」**。野心は残しつつ誇張を避ける。以下「Web版SwiftUI」表記はこの精密化された意味で読むこと。

> **2026-04-20訂正 (2) —「1言語完結」の限界**:
>
> Phase 6 本文は「TypeScript 1言語で書ける」を「HTML/CSS/JS の 3言語知識が不要」と同一視しているが、これは **構文統一** と **概念抽象化** を混同した誤り。
>
> DraftOleが実現しているのは「TypeScript 構文への統一」であり、「HTML/CSS/JS 概念からの解放」ではない。`div()` / `h1()` / `section()` の名前と意味は HTML タグそのものであり、利用者は依然として以下を理解している必要がある:
>
> - HTML要素セマンティクス（div / section / article / main の使い分け）
> - CSSプロパティとカスケード（flex, grid, padding, 詳細度）
> - JSイベントモデル（click, submit, DOM操作）
>
> SwiftUIが実現した「ネイティブUIプリミティブ（VStack/Text/Button）による HTML非参照」は、**DSLがHTMLを出力する以上、本質的に達成不可能**。`VStack` はHTMLに対応物がないから HTML を知らなくていいが、`div` は HTML そのものであり、HTML を知らずに書くことはできない。
>
> したがって「Web版SwiftUI」というアナロジーは **構文の類似性** までしか及ばず、**概念の独立性**（SwiftUIがiOSに対して持つような関係）は約束できない。
>
> **React/JSX との真の差別化軸は、この訂正を踏まえると次の2点に限定される**:
>
> 1. **構文統一の深さ**: CSS / JS までを同じチェーンで書ける（Reactは JSX = HTML のみ、CSS は別途）
> 2. **ランタイムの軽さ**: ゼロ依存、ビルドステップなしで `tsx` 直接実行可
>
> 「1言語完結」を売り文句にする場合は、**「HTML/CSS/JSを知らなくていい」と誤読されないよう**、「ツールチェーン・構文・型システムを TypeScript に統一できる」という正確な表現に置き換えるべき。

/*
 * Task 7.2 parity fixture: frozen snapshot of the original `appJs` template
 * literal that lived inside `examples/interactive/mvp-demo.ts` BEFORE task 6.2 rewrote the
 * demo to use the Vanilla Builder.
 *
 * Recovery command (do not modify this file without updating the snapshot
 * deliberately — this is the source of truth for mvp-demo-parity.test.ts):
 *
 *   git show 8d6bb8b~1:examples/interactive/mvp-demo.ts
 *
 * Extract the template literal assigned to `const appJs = \`...\`;` from the
 * "── JS ──" section (previously lines 188..240) and paste as raw JS below.
 * Comments in this header are JS line comments so the file itself is valid
 * JavaScript and can be consumed via fs.readFileSync in the parity test.
 */

...
import { CSSBorder, type BorderOptions } from '../../../../src/css/style/border/css-border.js';
...
describe('CSSBorder', () => {
  ...
  describe('setBorder（集約 setter）', () => {
    ...
    it('setBorder({width, style, color}) が border-color/border-style/border-width をアルファベット順で出力する', () => {
      const sut = makeSUT();
      sut.setBorder({ width: '1px', style: 'solid', color: '#333' });
      expect(sut.render()).toBe('border-color: #333;\nborder-style: solid;\nborder-width: 1px;');
    });

    it('setBorder({width}) は border-style/border-color/border-radius を出力しない', () => {
      const sut = makeSUT();
      sut.setBorder({ width: '1px' });
      const rendered = sut.render();
      expect(rendered).toBe('border-width: 1px;');
      expect(rendered).not.toContain('border-style');
      expect(rendered).not.toContain('border-color');
    ...
    it('setBorder() は何も出力しない', () => {
      const sut = makeSUT();
      sut.setBorder();
      expect(sut.render()).toBe('');
    });

    it('setBorder(undefined) は何も出力しない', () => {
      const sut = makeSUT();
      sut.setBorder(undefined);
      expect(sut.render()).toBe('');
    });
    ...
    it('setBorder が this を返す（identity check）', () => {
      const sut = makeSUT();
      const result = sut.setBorder({ width: '1px' });
      expect(result).toBe(sut);
    });

    it('setBorder が引数なしでも this を返す', () => {
      const sut = makeSUT();
      const result = sut.setBorder();
      expect(result).toBe(sut);
    });

    it('DEV モード: setBorderWidth 後に setBorder({width}) で DuplicateCssPropertyError(border-width)', () => {
      process.env.DRAFT_OLE_DEV = 'true';
      const sut = makeSUT();
      sut.setBorderWidth('1px');
      expect(() => sut.setBorder({ width: '2px' })).toThrow(DuplicateCssPropertyError);
      expect(() => {
        const sut2 = makeSUT();
        sut2.setBorderWidth('1px');
        sut2.setBorder({ width: '2px' });
      }).toThrow(/border-width/);
    });

    it('DEV モード: setBorder({width}) 連続呼び出しで DuplicateCssPropertyError(border-width)', () => {
      process.env.DRAFT_OLE_DEV = 'true';
      const sut = makeSUT();
      sut.setBorder({ width: '1px' });
      expect(() => sut.setBorder({ width: '2px' })).toThrow(DuplicateCssPropertyError);
      expect(() => {
        const sut2 = makeSUT();
        sut2.setBorder({ width: '1px' });
        sut2.setBorder({ width: '2px' });
      }).toThrow(/border-width/);
    });

    it('DEV モード: 単一 setBorder({width, style, color, radius}) は throw しない', () => {
      process.env.DRAFT_OLE_DEV = 'true';
      const sut = makeSUT();
      const options: BorderOptions = { width: '1px', style: 'solid', color: '#333', radius: '4px' };
      expect(() => sut.setBorder(options)).not.toThrow();
      const rendered = sut.render();
      expect(rendered).toContain('border-width: 1px');
      expect(rendered).toContain('border-style: solid');
      expect(rendered).toContain('border-color: #333');
      expect(rendered).toContain('border-radius: 4px');
    });

    it('setBorder({width}) は directional/corner ストレージを変更しない（Req 2.5）', () => {
      const sut = makeSUT();
      sut.setBorder({ width: '1px' });
      // _borderTopWidth など directional/corner は未設定のため、個別 setter を続けて呼んでも衝突しない
      expect(() => {
        sut.setBorderTopWidth('2px');
        sut.setBorderRightWidth('2px');
        sut.setBorderBottomWidth('2px');
        sut.setBorderLeftWidth('2px');
        sut.setBorderTopLeftRadius('3px');
        sut.setBorderTopRightRadius('3px');
        sut.setBorderBottomRightRadius('3px');
        sut.setBorderBottomLeftRadius('3px');
      }).not.toThrow();
      // render 出力にも directional/corner が setBorder によって出力されていないことを確認
      const fresh = makeSUT();
      fresh.setBorder({ width: '1px' });
      const rendered = fresh.render();
      expect(rendered).not.toContain('border-top-width');
      expect(rendered).not.toContain('border-right-width');
      expect(rendered).not.toContain('border-bottom-width');
      expect(rendered).not.toContain('border-left-width');
      expect(rendered).not.toContain('border-top-left-radius');
      expect(rendered).not.toContain('border-top-right-radius');
      expect(rendered).not.toContain('border-bottom-right-radius');
      expect(rendered).not.toContain('border-bottom-left-radius');
    });

    it('setBorder({radius}) は border-radius のみ出力する', () => {
      const sut = makeSUT();
      sut.setBorder({ radius: '4px' });
      expect(sut.render()).toBe('border-radius: 4px;');
    });
  });
});

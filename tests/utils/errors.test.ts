...
/**
 * ErrorsBranchTests: src/utils/errors.ts の未到達分岐網羅
 *
 * Public API Branch Coverage spec - タスク 2.1
 *
 * Requirements: 4.1, 4.2, 4.3, 4.4
 *
 * テスト対象:
 * - DraftOleError 直接インスタンス化時の throw（abstract class guard / errors.ts:155-159）
 * - Error.captureStackTrace 不在環境での DraftOleError / ModuleError 構築（errors.ts:162-164, 214-216）
 * - errorToString の入力バリアント網羅（errors.ts:260-262）
 *
 * 制約:
 * - 既存 38 件のテストは変更しない（このファイルの末尾に追記のみ）
 */
describe('ErrorsBranchTests (Req 4.1, 4.2, 4.3, 4.4)', () => {
  describe('DraftOleError 直接インスタンス化ガード (Req 4.2)', () => {
    it('DraftOleError を new で直接構築すると Error が throw される', async () => {
      // Arrange
      const { DraftOleError } = await import('../../src/utils/errors.js');

      // Act & Assert: abstract class なので直接 instantiation は禁止
      expect(() => {
        // @ts-expect-error - 抽象クラスの直接 new はコンパイルエラー / ランタイム throw
        new DraftOleError('attempt direct');
      }).toThrow(
        'DraftOleError is an abstract class and cannot be instantiated directly.',
      );
    });

    it('throw される値は Error インスタンス（DraftOleError ではない）', async () => {
      // Arrange
      const { DraftOleError } = await import('../../src/utils/errors.js');
      ...
      let caught: unknown;
      try {
        // @ts-expect-error - 抽象クラスの直接 new
        new DraftOleError('attempt direct');
      } catch (e) {
        caught = e;
      }

      // Assert: throw されるのは素の Error（DraftOleError ではない）
      expect(caught).toBeInstanceOf(Error);
      expect(caught).not.toBeInstanceOf(DraftOleError);
    });
  });

  describe('Error.captureStackTrace 不在 fallback (Req 4.3)', () => {
    // 元の Error.captureStackTrace 値を保持して afterEach で復元する。
    // vi.unstubAllGlobals は global 自体（globalThis.Error 等）を対象とする
    // ため、Error のメソッドプロパティには Object.defineProperty による
    // 直接 save/restore の方が確実。
    ...
    it('Error.captureStackTrace が undefined でも DraftOleError サブクラスは構築できる', async () => {
      // Arrange
      Object.defineProperty(Error, 'captureStackTrace', {
        value: undefined,
        configurable: true,
      ...
      class FallbackTestError extends DraftOleError {
        readonly code = 'fallback' as const;
        readonly module = 'html' as const;
        constructor(message: string) {
          super(message);
          this.name = 'FallbackTestError';
        }
      }

      // Act & Assert: captureStackTrace 不在でも例外が出ないこと
      expect(() => new FallbackTestError('fallback path')).not.toThrow();

      const error = new FallbackTestError('fallback path');
      expect(error).toBeInstanceOf(DraftOleError);
      expect(error.message).toBe('fallback path');
    });

    it('Error.captureStackTrace が undefined でも ModuleError サブクラス (HtmlError) は構築できる', async () => {
      // Arrange
      Object.defineProperty(Error, 'captureStackTrace', {
        value: undefined,
        configurable: true,
      ...
      // Act & Assert: ModuleError コンストラクタの captureStackTrace 分岐が
      // false 側を通っても throw しない
      expect(() => new HtmlError('invalidTag', 'fallback')).not.toThrow();

      const error = new HtmlError('invalidTag', 'fallback', { tagType: 'div' });
      expect(error.code).toBe('invalidTag');
      expect(error.module).toBe('html');
      expect(error.tagType).toBe('div');
    });

    it('Error.captureStackTrace が undefined でも ModuleError サブクラス (CssError) は構築できる', async () => {
      // Arrange
      Object.defineProperty(Error, 'captureStackTrace', {
        value: undefined,
        configurable: true,
      ...
      // Act & Assert
      expect(() => new CssError('invalidProperty', 'fallback')).not.toThrow();

      const error = new CssError('invalidProperty', 'fallback', {
        property: 'color',
      });
      expect(error.code).toBe('invalidProperty');
      expect(error.module).toBe('css');
      expect(error.property).toBe('color');
    });

    it('Error.captureStackTrace が undefined でも ModuleError サブクラス (JsError) は構築できる', async () => {
      // Arrange
      Object.defineProperty(Error, 'captureStackTrace', {
        value: undefined,
        configurable: true,
      ...
      // Act & Assert
      expect(() => new JsError('invalidSelector', 'fallback')).not.toThrow();

      const error = new JsError('invalidSelector', 'fallback', {
        selector: '#x',
      });
      expect(error.code).toBe('invalidSelector');
      expect(error.module).toBe('js');
      expect(error.selector).toBe('#x');
    });
  ...
  describe('errorToString 入力バリアント (Req 4.4)', () => {
    it('Error インスタンスは .message を返す', async () => {
      const { errorToString } = await import('../../src/utils/errors.js');
      expect(errorToString(new Error('x'))).toBe('x');
    });

    it('文字列はそのまま返す', async () => {
      const { errorToString } = await import('../../src/utils/errors.js');
      expect(errorToString('foo')).toBe('foo');
    });

    it('数値は String() による変換結果を返す', async () => {
      const { errorToString } = await import('../../src/utils/errors.js');
      expect(errorToString(42)).toBe('42');
    });

    it('プレーンオブジェクトは String() による変換結果（"[object Object]"）を返す', async () => {
      const { errorToString } = await import('../../src/utils/errors.js');
      expect(errorToString({ a: 1 })).toBe('[object Object]');
    });

    it('undefined は "undefined" 文字列を返す', async () => {
      const { errorToString } = await import('../../src/utils/errors.js');
      expect(errorToString(undefined)).toBe('undefined');
    });

    it('null は "null" 文字列を返す', async () => {
      const { errorToString } = await import('../../src/utils/errors.js');

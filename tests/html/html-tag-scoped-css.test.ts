...
import { div, p, h1, span, Text } from '../../src/html/tags/factories.js';
...
describe('DF-2: スコープCSS自動付与', () => {
  ...
  describe('スコープクラスのHTML付与', () => {
    it('CSSが設定された要素のHTMLにスコープクラスが付与される', () => {
      ...
      expect(html).toMatch(/class="[^"]*_(?:[a-z0-9-]+__)?[0-9a-f]{8}[^"]*"/);
    });
    ...
    it('CSS未設定の要素にはスコープクラスが付与されない', () => {
      ...
      expect(html).not.toMatch(/_(?:[a-z0-9-]+__)?[0-9a-f]{8}/);
    });
    ...
    it('異なる要素には異なるスコープクラスが付与される', () => {
      ...
      const html = parent.render();
      const matches = html.match(/_(?:[a-z0-9-]+__)?[0-9a-f]{8}/g) ?? [];
  ...
  describe('スコープ付きCSS出力', () => {
    it('collectCssStyleString がスコープ付きCSS（._hash { ... }）を返す', () => {
      ...
      expect(css).toMatch(/\._(?:[a-z0-9-]+__)?[0-9a-f]{8}\s*\{[^}]*font-size: 16px/);
    });
    ...
    it('HTMLのクラスとCSSのセレクタが一致する', () => {
      ...
      // HTMLからスコープクラスを抽出
      const htmlClassMatch = html.match(/(_(?:[a-z0-9-]+__)?[0-9a-f]{8})/);
      expect(htmlClassMatch).not.toBeNull();

      // CSSにも同じクラスが含まれる
      const scopeClass = htmlClassMatch![1];
  ...
  describe('統合シナリオ', () => {
    it('Root経由でHTML+CSSを出力し、スコープが一致する', () => {
      ...
      // HTMLにスコープクラスが含まれる
      expect(html).toMatch(/_(?:[a-z0-9-]+__)?[0-9a-f]{8}/);
      // CSSにスコープセレクタが含まれる

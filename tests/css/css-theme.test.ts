...
describe('6.5: CSS変数（createTheme）', () => {
  ...
  describe('Root 統合', () => {
    it('theme.css を new Root({ css: [theme.css] }) で注入し、collectCssStyleString に含まれる', () => {
      const theme = createTheme({ bg: '#0b1220', accent: '#7c5cff' });
      const root = new Root({ css: [theme.css] });
  ...
  describe('セキュリティ: CSS値サニタイズ', () => {
    it('url(javascript:...) を含む変数は CSS 出力に含まれない', () => {
      const theme = createTheme({ bg: 'url(javascript:alert(1))' });
      expect(theme.css).not.toContain('url(javascript:');
      expect(theme.css).not.toContain('--bg:');
    });

    it('url(javascript:...) を含む変数が複数ある場合、危険な値のみ除外される', () => {
      const theme = createTheme({
      ...
      expect(theme.css).not.toContain('url(javascript:');
      expect(theme.css).not.toContain('--bg:');
      expect(theme.css).toContain('--accent: #7c5cff;');
    });

    it('expression() を含む変数は CSS 出力に含まれない', () => {
      const theme = createTheme({ color: 'expression(alert(1))' });
      expect(theme.css).not.toContain('expression(');
      expect(theme.css).not.toContain('--color:');
    });
  });
